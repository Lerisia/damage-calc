// Speed-ability comparison harness. Reads /tmp/scenarios_speed.json
// from tools/showdown-compare/gen_speed.mjs and replays each scenario
// through DamageCalculator, comparing the 16-roll spread against
// @smogon/calc. Exercises speed-modifying abilities (Swift Swim etc.)
// through the speed-based-power moves Gyro Ball / Electro Ball.
import 'dart:convert';
import 'dart:io';
...
const _natureMap = {
  'Jolly': Nature.jolly, 'Timid': Nature.timid, 'Adamant': Nature.adamant,
  'Modest': Nature.modest, 'Brave': Nature.brave, 'Quiet': Nature.quiet,
  'Hardy': Nature.hardy,
};
const _statusMap = {
  'par': StatusCondition.paralysis, 'brn': StatusCondition.burn,
  'psn': StatusCondition.poison,
};

void main() {
  test('Showdown speed-ability compare', () async {
    ...
    final scenarios = (jsonDecode(
        File('/tmp/scenarios_speed.json').readAsStringSync()) as List)
    ...
    for (final s in scenarios) {
      ...
      final atk = BattlePokemonState()
        ...
        ..status = _statusMap[s['atkStatus']] ?? StatusCondition.none
      ...
      final def = BattlePokemonState()
        ...
        ..status = _statusMap[s['defStatus']] ?? StatusCondition.none;
      ...
      if (ours.length == 16 &&
        ...
        diffs.add(
          'DIFF ${s['atkSpec']}(${s['atkAbility']},${s['atkNature']},'
          'spe+${s['atkSpeedBoost']},${s['atkStatus']}) ${s['move']} → '
          '${s['defSpec']}(${s['defAbility']},spe+${s['defSpeedBoost']})  '
    ...
    // ignore: avoid_print
    print('speed-ability matched=$matched / total=$total');
    if (diffs.isNotEmpty) {
      // ignore: avoid_print
