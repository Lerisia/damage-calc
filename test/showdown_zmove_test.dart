// Z-Move comparison harness. Pairs each move with its matching
// Z-Crystal so both calcs converge on the same Z-move BP table.
import 'dart:convert';
import 'dart:io';
...
void main() {
  test('Showdown Z-move compare', () async {
    ...
    final scenarios = (jsonDecode(
        File('/tmp/scenarios_zmove.json').readAsStringSync()) as List)
    ...
    for (final s in scenarios) {
      ...
      final atk = BattlePokemonState()
        ...
        ..selectedItem = _itemSlug(s['item'] as String)
        ..moves = [move, null, null, null]
        ..criticals = [s['isCrit'] == true, false, false, false]
        ..zMoves = [true, false, false, false];
      ...
      if (ours.length == 16 &&
        ...
        diffs.add(
          'DIFF ${s['atkSpec']} (${atk.selectedItem}, ${s['move']}) → ${s['defSpec']}\n'
          '  ours:    $ours\n'
          '  showdown:$theirs',
        );
      }
    ...
    // ignore: avoid_print
    print('zmove matched=$matched / total=$total');
    if (diffs.isNotEmpty) {
      // ignore: avoid_print
      print('First 5 diffs:');
