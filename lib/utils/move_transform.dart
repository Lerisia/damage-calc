...
/// Fling: power is the held item's own fling power (type stays Dark,
/// category stays physical). The table is generated from Showdown's
/// item data (`lib/data/fling_powers.dart`); a held item absent from
/// it — gems, Z-Crystals, the Weather-trio orbs — makes the move fail,
/// as does holding nothing, so both come out as power 0.
Move _applyFlingPower(Move move, String? heldItem) {
  if (move.name != 'Fling') return move;
  if (heldItem == null || heldItem.isEmpty) return move.copyWith(power: 0);
  return move.copyWith(power: kFlingPowers[heldItem] ?? 0);
}
