...
class _DamageCalculatorScreenState extends State<DamageCalculatorScreen>
  ...
  /// Sprite style picker. Lists the three styles with a radio for the
  /// active pick, and on mobile a per-style install state + Download
  /// + Import + Remove triplet so the user can manage their imported
  /// packs without leaving the dialog. Listens on both [SpriteService]
  /// (style switches) and [SpritePackManager] (install/uninstall) so
  /// any change refreshes the dialog in place.
  void _showSpriteStyleDialog() {
    showDialog(
      context: context,
      builder: (ctx) => const _SpriteStyleDialog(),
    );
  }
...
/// nothing to install.
class _SpriteStyleDialog extends StatefulWidget {
  const _SpriteStyleDialog();

  @override
  State<_SpriteStyleDialog> createState() => _SpriteStyleDialogState();
}

class _SpriteStyleDialogState extends State<_SpriteStyleDialog> {
