// Auto-extracted from Showdown trainer sprite folder.
// Used by the trainer-card picker to enumerate which sprite
// keys to render in the grid — mobile FileImage reads from
// the imported sprite pack cache, web NetworkImage fetches
// from play.pokemonshowdown.com/sprites/trainers/<key>.png.
library;

const List<String> trainerSpriteKeys = [
  'aaron',
  'aarune',
  'acerola',
  'acerola-masters',
  'acerola-masters2',
  'acerola-masters3',
  'acetrainer',
  'acetrainercouple',
  'acetrainercouple-gen3',
  'acetrainerf',
  'acetrainerf-gen1',
  'acetrainerf-gen1rb',
  'acetrainerf-gen2',
  'acetrainerf-gen3',
  'acetrainerf-gen3rs',
  'acetrainerf-gen4',
  'acetrainerf-gen4dp',
  'acetrainerf-gen6',
  'acetrainerf-gen6xy',
  'acetrainerf-gen7',
  'acetrainer-gen1',
  'acetrainer-gen1rb',
  'acetrainer-gen2',
  'acetrainer-gen3',
  'acetrainer-gen3jp',
  'acetrainer-gen3rs',
  'acetrainer-gen4',
  'acetrainer-gen4dp',
  'acetrainer-gen6',
  'acetrainer-gen6xy',
  'acetrainer-gen7',
  'acetrainersnow',
  'acetrainersnowf',
  'adaman',
  'adaman-masters',
  'aetheremployee',
  'aetheremployeef',
  'aetherfoundation',
  'aetherfoundation2',
  'aetherfoundationf',
  'agatha-gen1',
  'agatha-gen1rb',
  'agatha-gen3',
  'agatha-lgpe',
  'akari',
  'akari-isekai',
  'alain',
  'alder',
  'alec-anime',
  'allister',
  'allister-masters',
  'allister-unmasked',
  'amarys',
  'amelia-shuffle',
  'anabel',
  'anabel-gen3',
  'anabel-gen7',
  'ansha',
  'ansha-cook',
  'anthe',
  'anthea',
  'anvin',
  'aquagrunt',
  'aquagruntf',
  'aquagruntf-rse',
  'aquagrunt-rse',
  'aquasuit',
  'archer',
  'archie-gen3',
  'archie-gen6',
  'archie-usum',
  'arezu',
  'argenta',
  'ariana',
  'arlo',
  'aromalady',
  'aromalady-gen3',
  'aromalady-gen3rs',
  'aromalady-gen6',
  'artist',
  'artistf-gen6',
  'artist-gen4',
  'artist-gen6',
  'artist-gen8',
  'artist-gen9',
  'arven-s',
  'arven-v',
  'ash',
  'ash-alola',
  'ash-capbackward',
  'ash-hoenn',
  'ash-johto',
  'ash-kalos',
  'ash-sinnoh',
  'ash-unova',
  'atticus',
  'avery',
  'az',
  'az-lza',
  'backers',
  'backersf',
  'backpacker',
  'backpackerf',
  'backpacker-gen6',
  'backpacker-gen8',
  'backpacker-gen9',
  'baker',
  'ballguy',
  'baoba',
  'barry',
  'barry-masters',
  'battlegirl',
  'battlegirl-gen3',
  'battlegirl-gen4',
  'battlegirl-gen6',
  'battlegirl-gen6xy',
  'bea',
  'bea-masters',
  'beauty',
  'beauty-gen1',
  'beauty-gen1rb',
  'beauty-gen2',
  'beauty-gen2jp',
  'beauty-gen3',
  'beauty-gen3rs',
  'beauty-gen4dp',
  'beauty-gen5bw2',
  'beauty-gen6',
  'beauty-gen6xy',
  'beauty-gen7',
  'beauty-gen8',
  'beauty-gen9',
  'beauty-masters',
  'bede',
  'bede-leader',
  'bede-masters',
  'bellelba',
  'bellepa',
  'bellhop',
  'bellis',
  'benga',
  'beni',
  'beni-ninja',
  'bertha',
  'bianca',
  'bianca-masters',
  'bianca-pwt',
  'biker',
  'biker-gen1',
  'biker-gen1rb',
  'biker-gen2',
  'biker-gen3',
  'biker-gen4',
  'bill',
  'bill-gen3',
  'birch',
  'birch-gen3',
  'birdkeeper',
  'birdkeeper-gen1',
  'birdkeeper-gen1rb',
  'birdkeeper-gen2',
  'birdkeeper-gen3',
  'birdkeeper-gen3rs',
  'birdkeeper-gen4dp',
  'birdkeeper-gen6',
  'blackbelt',
  'blackbelt-gen1',
  'blackbelt-gen1rb',
  'blackbelt-gen2',
  'blackbelt-gen3',
  'blackbelt-gen3rs',
  'blackbelt-gen4',
  'blackbelt-gen4dp',
  'blackbelt-gen6',
  'blackbelt-gen7',
  'blackbelt-gen8',
  'blackbelt-gen9',
  'blaine',
  'blaine-gen1',
  'blaine-gen1rb',
  'blaine-gen2',
  'blaine-gen3',
  'blaine-lgpe',
  'blanche',
  'blanche-casual',
  'blue',
  'blue-gen1',
  'blue-gen1champion',
  'blue-gen1rb',
  'blue-gen1rbchampion',
  'blue-gen1rbtwo',
  'blue-gen1two',
  'blue-gen2',
  'blue-gen3',
  'blue-gen3champion',
  'blue-gen3two',
  'blue-gen7',
  'blue-lgpe',
  'blue-masters',
  'blue-masters2',
  'boarder',
  'boarder-gen2',
  'bodybuilderf-gen9',
  'bodybuilder-gen9',
  'brandon',
  'brandon-gen3',
  'brassius',
  'brawly',
  'brawly-gen3',
  'brawly-gen6',
  'brendan',
  'brendan-contest',
  'brendan-e',
  'brendan-gen3',
  'brendan-gen3rs',
  'brendan-masters',
  'brendan-masters2',
  'brendan-rs',
  'briar',
  'brigette',
  'brock',
  'brock-gen1',
  'brock-gen1rb',
  'brock-gen2',
  'brock-gen3',
  'brock-lgpe',
  'brock-masters',
  'bruno',
  'bruno-gen1',
  'bruno-gen1rb',
  'bruno-gen2',
  'bruno-gen3',
  'brycen',
  'brycenman',
  'bryony',
  'buck',
  'bugcatcher',
  'bugcatcher-gen1',
  'bugcatcher-gen1rb',
  'bugcatcher-gen2',
  'bugcatcher-gen3',
  'bugcatcher-gen3rs',
  'bugcatcher-gen4dp',
  'bugcatcher-gen6',
  'bugmaniac-gen3',
  'bugmaniac-gen6',
  'bugsy',
  'bugsy-gen2',
  'bugsy-masters',
  'burgh',
  'burgh-masters',
  'burglar',
  'burglar-gen1',
  'burglar-gen1rb',
  'burglar-gen2',
  'burglar-gen3',
  'burglar-lgpe',
  'burnet',
  'burnet-radar',
  'butler',
  'byron',
  'cabbie',
  'cabbie-gen9',
  'cafemaster',
  'caitlin',
  'caitlin-gen4',
  'caitlin-masters',
  'calaba',
  'calem',
  'calem-masters',
  'cameraman',
  'cameraman-gen6',
  'cameraman-gen8',
  'camper',
  'camper-gen2',
  'camper-gen3',
  'camper-gen3rs',
  'camper-gen6',
  'canari',
  'candela',
  'candela-casual',
  'candice',
  'candice-masters',
  'caraliss',
  'caretaker',
  'carmine',
  'carmine-festival',
  'cedricjuniper',
  'celio',
  'channeler-gen1',
  'channeler-gen1rb',
  'channeler-gen3',
  'channeler-lgpe',
  'charm',
  'charon',
  'chase',
  'chef',
  'cheren',
  'cheren-gen5bw2',
  'cheren-masters',
  'cheryl',
  'chili',
  'choy',
  'christoph',
  'chuck',
  'chuck-gen2',
  'cilan',
  'clair',
  'clair-gen2',
  'clair-masters',
  'clavell-s',
  'clay',
  'clemont',
  'clerk',
  'clerk-boss',
  'clerkf',
  'clerkf-gen8',
  'clerk-gen8',
  'clerk-unite',
  'cliff',
  'clive-v',
  'clover',
  'clown',
  'cogita',
  'coin',
  'collector',
  'collector-gen3',
  'collector-gen6',
  'collector-gen7',
  'collector-masters',
  'colress',
  'colress-gen7',
  'colza',
  'concordia',
  'cook',
  'cook-gen7',
  'cook-gen9',
  'corbeau',
  'courier',
  'courtney',
  'courtney-gen3',
  'cowgirl',
  'crasherwake',
  'cress',
  'crispin',
  'crushgirl-gen3',
  'crushkin-gen3',
  'cueball-gen1',
  'cueball-gen1rb',
  'cueball-gen3',
  'curtis',
  'cyclist',
  'cyclistf',
  'cyclistf-gen4',
  'cyclist-gen4',
  'cyllene',
  'cynthia',
  'cynthia-anime',
  'cynthia-anime2',
  'cynthia-gen4',
  'cynthia-gen7',
  'cynthia-masters',
  'cynthia-masters2',
  'cynthia-masters3',
  'cynthia-masters4',
  'cyrano',
  'cyrus',
  'cyrus-masters',
  'dagero',
  'dahlia',
  'daisy',
  'daisy-gen3',
  'dana',
  'dancer',
  'dancer-gen7',
  'dancer-gen8',
  'darach',
  'darach-caitlin',
  'dawn',
  'dawn-contest',
  'dawn-gen4pt',
  'dawn-masters',
  'dawn-masters2',
  'dawn-masters3',
  'delinquent',
  'delinquentf2-gen9',
  'delinquentf-gen9',
  'delinquent-gen9',
  'dendra',
  'depotagent',
  'dexio',
  'dexio-gen6',
  'diamondclanmember',
  'diantha',
  'diantha-masters',
  'diantha-masters2',
  'doctor',
  'doctorf-gen8',
  'doctor-gen8',
  'doubleteam',
  'dragontamer',
  'dragontamer-gen3',
  'dragontamer-gen6',
  'dragontamer-gen9',
  'drake-gen3',
  'drasna',
  'drayden',
  'drayton',
  'dulse',
  'elaine',
  'elesa',
  'elesa-gen5bw2',
  'elesa-masters',
  'elesa-masters2',
  'elesa-masters3',
  'elio',
  'elio-masters',
  'elio-usum',
  'elm',
  'emma',
  'emma-lza',
  'emmet',
  'emmet-masters',
  'engineer-gen1',
  'engineer-gen1rb',
  'engineer-gen3',
  'erbie-unite',
  'eri',
  'erika',
  'erika-gen1',
  'erika-gen1rb',
  'erika-gen2',
  'erika-gen3',
  'erika-lgpe',
  'erika-masters',
  'erika-masters2',
  'erika-masters3',
  'essentia',
  'ethan',
  'ethan-gen2',
  'ethan-gen2c',
  'ethan-masters',
  'ethan-pokeathlon',
  'eusine',
  'eusine-gen2',
  'evelyn',
  'expertf-gen3',
  'expertf-gen6',
  'expert-gen3',
  'expert-gen6',
  'faba',
  'fairytalegirl',
  'falkner',
  'falkner-gen2',
  'fantina',
  'fennel',
  'firebreather',
  'firebreather-gen2',
  'firefighter',
  'fisher-gen8',
  'fisherman',
  'fisherman-gen1',
  'fisherman-gen1rb',
  'fisherman-gen2jp',
  'fisherman-gen3',
  'fisherman-gen3rs',
  'fisherman-gen4',
  'fisherman-gen6',
  'fisherman-gen6xy',
  'fisherman-gen7',
  'flannery',
  'flannery-gen3',
  'flannery-gen6',
  'flaregrunt',
  'flaregruntf',
  'flint',
  'florian-bb',
  'florian-festival',
  'florian-s',
  'freediver',
  'furisodegirl-black',
  'furisodegirl-blue',
  'furisodegirl-pink',
  'furisodegirl-white',
  'gaeric',
  'galacticgrunt',
  'galacticgruntf',
  'gambler',
  'gambler-gen1',
  'gambler-gen1rb',
  'gamer-gen3',
  'garcon',
  'gardener',
  'gardenia',
  'gardenia-masters',
  'geeta',
  'gentleman',
  'gentleman-gen1',
  'gentleman-gen1rb',
  'gentleman-gen2',
  'gentleman-gen3',
  'gentleman-gen3rs',
  'gentleman-gen4',
  'gentleman-gen4dp',
  'gentleman-gen6',
  'gentleman-gen6xy',
  'gentleman-gen7',
  'gentleman-gen8',
  'gentleman-lgpe',
  'ghetsis',
  'ghetsis-gen5bw',
  'giacomo',
  'ginchiyo-conquest',
  'ginter',
  'giovanni',
  'giovanni-gen1',
  'giovanni-gen1rb',
  'giovanni-gen3',
  'giovanni-lgpe',
  'giovanni-masters',
  'glacia',
  'glacia-gen3',
  'gladion',
  'gladion-masters',
  'gladion-stance',
  'gloria',
  'gloria-dojo',
  'gloria-league',
  'gloria-masters',
  'gloria-tundra',
  'golfer',
  'gordie',
  'grace',
  'grant',
  'green',
  'greta',
  'greta-gen3',
  'grimsley',
  'grimsley-gen7',
  'grimsley-masters',
  'grisham',
  'grusha',
  'guitarist',
  'guitarist-gen2',
  'guitarist-gen3',
  'guitarist-gen4',
  'guitarist-gen6',
  'gurkinn',
  'guzma',
  'guzma-masters',
  'gwynn',
  'hala',
  'hanbei-conquest',
  'hapu',
  'harlequin',
  'harmony',
  'hassel',
  'hau',
  'hau-masters',
  'hau-stance',
  'hayley',
  'heath',
  'hero2-conquest',
  'hero-conquest',
  'heroine2-conquest',
  'heroine-conquest',
  'hexmaniac-gen3',
  'hexmaniac-gen3jp',
  'hexmaniac-gen6',
  'hiker',
  'hiker-gen1',
  'hiker-gen1rb',
  'hiker-gen2',
  'hiker-gen3',
  'hiker-gen3rs',
  'hiker-gen4',
  'hiker-gen6',
  'hiker-gen7',
  'hiker-gen8',
  'hiker-gen9',
  'hilbert',
  'hilbert-masters',
  'hilbert-masters2',
  'hilbert-wonderlauncher',
  'hilda',
  'hilda-masters',
  'hilda-masters2',
  'hilda-masters3',
  'hilda-wonderlauncher',
  'hooligans',
  'hoopster',
  'hop',
  'hop-masters',
  'hugh',
  'hyde',
  'idol',
  'ilima',
  'infielder',
  'ingo',
  'ingo-hisui',
  'ingo-masters',
  'interviewers',
  'interviewers-gen3',
  'interviewers-gen6',
  'iono',
  'iono-masters',
  'iono-masters2',
  'irida',
  'irida-masters',
  'iris',
  'iris-gen5bw2',
  'iris-masters',
  'iscan',
  'ivor',
  'jacinthe',
  'jacq',
  'jamie',
  'janine',
  'janine-gen2',
  'janitor',
  'janitor-gen7',
  'janitor-gen9',
  'jasmine',
  'jasmine-contest',
  'jasmine-gen2',
  'jasmine-masters',
  'jasmine-masters2',
  'jessiejames-gen1',
  'jogger',
  'johanna',
  'johanna-contest',
  'jrtrainerf-gen1',
  'jrtrainerf-gen1rb',
  'jrtrainer-gen1',
  'jrtrainer-gen1rb',
  'juan',
  'juan-gen3',
  'juggler',
  'juggler-gen1',
  'juggler-gen1rb',
  'juggler-gen2',
  'juggler-gen3',
  'juliana-bb',
  'juliana-festival',
  'juliana-s',
  'juniper',
  'jupiter',
  'kabu',
  'kahili',
  'kamado',
  'kamado-armor',
  'karen',
  'karen-gen2',
  'katy',
  'kiawe',
  'kieran',
  'kieran-champion',
  'kieran-festival',
  'kimonogirl',
  'kimonogirl-gen2',
  'kindler-gen3',
  'kindler-gen6',
  'klara',
  'kofu',
  'koga',
  'koga-gen1',
  'koga-gen1rb',
  'koga-gen2',
  'koga-gen3',
  'koga-lgpe',
  'korrina',
  'korrina-masters',
  'kris',
  'kris-gen2',
  'kris-masters',
  'kukui',
  'kukui-stand',
  'kunoichi2-conquest',
  'kunoichi-conquest',
  'kurt',
  'lacey',
  'lady',
  'lady-gen3',
  'lady-gen3rs',
  'lady-gen4',
  'lady-gen6',
  'lady-gen6oras',
  'lana',
  'lana-masters',
  'lance',
  'lance-gen1',
  'lance-gen1rb',
  'lance-gen2',
  'lance-gen3',
  'lance-lgpe',
  'lance-masters',
  'lance-masters2',
  'lanette',
  'larry',
  'lass',
  'lass-gen1',
  'lass-gen1rb',
  'lass-gen2',
  'lass-gen3',
  'lass-gen3rs',
  'lass-gen4',
  'lass-gen4dp',
  'lass-gen6',
  'lass-gen6oras',
  'lass-gen7',
  'lass-gen8',
  'laventon',
  'laventon2',
  'leaf-gen3',
  'leaf-masters',
  'leaf-masters2',
  'leaguestaff',
  'leaguestafff',
  'lebanne',
  'lenora',
  'leon',
  'leon-masters',
  'leon-masters2',
  'leon-tower',
  'li',
  'lian',
  'lida',
  'liko',
  'lillie',
  'lillie-masters',
  'lillie-masters2',
  'lillie-masters3',
  'lillie-z',
  'linebacker',
  'lisia',
  'lisia-masters',
  'liza',
  'liza-gen6',
  'liza-masters',
  'lorelei-gen1',
  'lorelei-gen1rb',
  'lorelei-gen3',
  'lorelei-lgpe',
  'ltsurge',
  'ltsurge-gen1',
  'ltsurge-gen1rb',
  'ltsurge-gen2',
  'ltsurge-gen3',
  'lucas',
  'lucas-contest',
  'lucas-gen4pt',
  'lucian',
  'lucy',
  'lucy-gen3',
  'lusamine',
  'lusamine-masters',
  'lusamine-nihilego',
  'lyra',
  'lyra-masters',
  'lyra-masters2',
  'lyra-pokeathlon',
  'lysandre',
  'lysandre-masters',
  'mable',
  'madame',
  'madame-gen4',
  'madame-gen4dp',
  'madame-gen6',
  'madame-gen7',
  'madame-gen8',
  'magmagrunt',
  'magmagruntf',
  'magmagruntf-rse',
  'magmagrunt-rse',
  'magmasuit',
  'magnolia',
  'magnus',
  'mai',
  'maid',
  'maid-gen4',
  'maid-gen6',
  'mallow',
  'mallow-masters',
  'malva',
  'marley',
  'marley-masters',
  'marlon',
  'marnie',
  'marnie-league',
  'marnie-masters',
  'marnie-masters2',
  'marnie-masters3',
  'mars',
  'marshal',
  'masamune-conquest',
  'mateo',
  'matt',
  'matt-gen3',
  'maxie-gen3',
  'maxie-gen6',
  'may',
  'may-contest',
  'may-e',
  'may-gen3',
  'may-gen3rs',
  'maylene',
  'may-masters',
  'may-masters2',
  'may-masters3',
  'may-masters4',
  'may-rs',
  'medium',
  'medium-gen2jp',
  'mela',
  'melli',
  'melony',
  'miku-fairy',
  'miku-fire',
  'miku-flying',
  'miku-ghost',
  'miku-grass',
  'miku-ground',
  'miku-ice',
  'miku-psychic',
  'miku-water',
  'milo',
  'mina',
  'mina-lgpe',
  'mina-masters',
  'mira',
  'miriam',
  'mirror',
  'misty',
  'misty-gen1',
  'misty-gen1rb',
  'misty-gen2',
  'misty-gen3',
  'misty-lgpe',
  'misty-masters',
  'model-gen8',
  'mohn',
  'mohn-anime',
  'molayne',
  'mom-alola',
  'mom-hoenn',
  'mom-johto',
  'mom-paldea',
  'mom-unova',
  'mom-unova2',
  'morgan',
  'morty',
  'morty-gen2',
  'morty-masters',
  'morty-masters2',
  'morty-masters3',
  'mrbriney',
  'mrfuji-gen3',
  'mrstone',
  'musician',
  'musician-gen8',
  'musician-gen9',
  'mustard',
  'mustard-champion',
  'mustard-master',
  'n',
  'nancy',
  'nanu',
  'nate',
  'nate-masters',
  'nate-pokestar',
  'nate-pokestar3',
  'nate-wonderlauncher',
  'naveen',
  'nemona-masters',
  'nemona-s',
  'nemona-v',
  'neroli',
  'nessa',
  'nessa-masters',
  'ninjaboy',
  'ninjaboy-gen3',
  'ninjaboy-gen6',
  'nita',
  'n-masters',
  'n-masters2',
  'n-masters3',
  'nobunaga-conquest',
  'noland',
  'noland-gen3',
  'norman',
  'norman-gen3',
  'norman-gen6',
  'nurse',
  'nurseryaide',
  'oak',
  'oak-gen1',
  'oak-gen1rb',
  'oak-gen2',
  'oak-gen3',
  'officer-gen2',
  'officeworker',
  'officeworkerf',
  'officeworkerf-gen9',
  'officeworker-gen9',
  'ogreclan',
  'oichi-conquest',
  'oldcouple-gen3',
  'oleana',
  'olivia',
  'olympia',
  'opal',
  'ortega',
  'owner',
  'painter-gen3',
  'palina',
  'palmer',
  'parasollady',
  'parasollady-gen3',
  'parasollady-gen4',
  'parasollady-gen6',
  'paulo-masters',
  'paxton',
  'pearlclanmember',
  'penny',
  'peonia',
  'peony',
  'peony-league',
  'perrin',
  'pesselle',
  'petrel',
  'phil',
  'phillipe',
  'phoebe-gen3',
  'phoebe-gen6',
  'phoebe-masters',
  'phorus-unite',
  'phyco',
  'picnicker',
  'picnicker-gen2',
  'picnicker-gen3',
  'picnicker-gen3rs',
  'picnicker-gen6',
  'piers',
  'piers-league',
  'piers-masters',
  'pilot',
  'plasmagrunt',
  'plasmagruntf',
  'plasmagruntf-gen5bw',
  'plasmagrunt-gen5bw',
  'playerf-go',
  'player-go',
  'plumeria',
  'plumeria-league',
  'pokefan',
  'pokefanf',
  'pokefanf-gen2',
  'pokefanf-gen3',
  'pokefanf-gen4',
  'pokefanf-gen6',
  'pokefanf-gen6xy',
  'pokefan-gen2',
  'pokefan-gen3',
  'pokefan-gen4',
  'pokefan-gen6',
  'pokefan-gen6xy',
  'pokekid',
  'pokekidf-gen8',
  'pokekid-gen8',
  'pokemaniac',
  'pokemaniac-gen1',
  'pokemaniac-gen1rb',
  'pokemaniac-gen2',
  'pokemaniac-gen3',
  'pokemaniac-gen3rs',
  'pokemaniac-gen6',
  'pokemaniac-gen9',
  'pokemonbreeder',
  'pokemonbreederf',
  'pokemonbreederf-gen3',
  'pokemonbreederf-gen3frlg',
  'pokemonbreederf-gen4',
  'pokemonbreederf-gen6',
  'pokemonbreederf-gen6xy',
  'pokemonbreederf-gen7',
  'pokemonbreederf-gen8',
  'pokemonbreeder-gen3',
  'pokemonbreeder-gen4',
  'pokemonbreeder-gen6',
  'pokemonbreeder-gen6xy',
  'pokemonbreeder-gen7',
  'pokemonbreeder-gen8',
  'pokemoncenterlady',
  'pokemonranger',
  'pokemonrangerf',
  'pokemonrangerf-gen3',
  'pokemonrangerf-gen3rs',
  'pokemonrangerf-gen4',
  'pokemonrangerf-gen6',
  'pokemonrangerf-gen6xy',
  'pokemonranger-gen3',
  'pokemonranger-gen3rs',
  'pokemonranger-gen4',
  'pokemonranger-gen6',
  'pokemonranger-gen6xy',
  'policeman',
  'policeman-gen4',
  'policeman-gen7',
  'policeman-gen8',
  'poppy',
  'postman',
  'preschooler',
  'preschoolerf',
  'preschoolerf-gen6',
  'preschoolerf-gen7',
  'preschooler-gen6',
  'preschooler-gen7',
  'preschoolers',
  'proton',
  'pryce',
  'pryce-gen2',
  'psychic',
  'psychicf',
  'psychicf-gen3',
  'psychicf-gen3rs',
  'psychicf-gen4',
  'psychicfjp-gen3',
  'psychic-gen1',
  'psychic-gen1rb',
  'psychic-gen2',
  'psychic-gen3',
  'psychic-gen3rs',
  'psychic-gen4',
  'psychic-gen6',
  'psychic-lgpe',
  'punkgirl',
  'punkgirl-gen7',
  'punkgirl-masters',
  'punkguy',
  'punkguy-gen7',
  'raifort',
  'raihan',
  'raihan-masters',
  'railstaff',
  'rainbowrocketgrunt',
  'rainbowrocketgruntf',
  'ramos',
  'rancher',
  'ranmaru-conquest',
  'red',
  'red-gen1',
  'red-gen1main',
  'red-gen1rb',
  'red-gen1title',
  'red-gen2',
  'red-gen3',
  'red-gen7',
  'red-lgpe',
  'red-masters',
  'red-masters2',
  'red-masters3',
  'rei',
  'rei-isekai',
  'reporter',
  'reporter-gen6',
  'reporter-gen8',
  'rhi',
  'richboy',
  'richboy-gen3',
  'richboy-gen4',
  'richboy-gen6',
  'richboy-gen6xy',
  'rika',
  'riley',
  'risingstar',
  'risingstarf',
  'risingstarf-gen6',
  'risingstar-gen6',
  'rita',
  'river',
  'roark',
  'rocker-gen1',
  'rocker-gen1rb',
  'rocker-gen3',
  'rocketexecutivef-gen2',
  'rocketexecutive-gen2',
  'rocket-gen1',
  'rocket-gen1rb',
  'rocketgrunt',
  'rocketgruntf',
  'rocketgruntf-gen2',
  'rocketgrunt-gen2',
  'rollerskater',
  'rollerskaterf',
  'rood',
  'rosa',
  'rosa-masters',
  'rosa-masters2',
  'rosa-masters3',
  'rosa-pokestar',
  'rosa-pokestar2',
  'rosa-pokestar3',
  'rosa-wonderlauncher',
  'rose',
  'rose-zerosuit',
  'roughneck',
  'roughneck-gen4',
  'rowan',
  'roxanne',
  'roxanne-gen3',
  'roxanne-gen6',
  'roxanne-masters',
  'roxie',
  'roxie-masters',
  'roy',
  'ruffian',
  'ruinmaniac',
  'ruinmaniac-gen3',
  'ruinmaniac-gen3rs',
  'ruinmaniac-gen6',
  'rye',
  'ryme',
  'ryuki',
  'sabi',
  'sabrina',
  'sabrina-frlg',
  'sabrina-gen1',
  'sabrina-gen1rb',
  'sabrina-gen2',
  'sabrina-gen3',
  'sabrina-lgpe',
  'sabrina-masters',
  'sada',
  'sada-ai',
  'sage',
  'sage-gen2',
  'sage-gen2jp',
  'saguaro',
  'sailor',
  'sailor-gen1',
  'sailor-gen1rb',
  'sailor-gen2',
  'sailor-gen3',
  'sailor-gen3jp',
  'sailor-gen3rs',
  'sailor-gen6',
  'salvatore',
  'samsonoak',
  'sanqua',
  'saturn',
  'sbcmember',
  'schoolboy',
  'schoolboy-gen2',
  'schoolgirl',
  'schoolkid',
  'schoolkidf',
  'schoolkidf-gen3',
  'schoolkidf-gen4',
  'schoolkidf-gen6',
  'schoolkidf-gen8',
  'schoolkid-gen3',
  'schoolkid-gen4',
  'schoolkid-gen4dp',
  'schoolkid-gen6',
  'schoolkid-gen8',
  'scientist',
  'scientistf',
  'scientistf-gen6',
  'scientist-gen1',
  'scientist-gen1rb',
  'scientist-gen2',
  'scientist-gen3',
  'scientist-gen4',
  'scientist-gen4dp',
  'scientist-gen6',
  'scientist-gen7',
  'scientist-gen9',
  'scott',
  'scottie-masters',
  'scubadiver',
  'securitycorps',
  'securitycorpsf',
  'selene',
  'selene-masters',
  'selene-masters2',
  'selene-usum',
  'serena',
  'serena-anime',
  'serena-masters',
  'serena-masters2',
  'serena-masters3',
  'shadowtriad',
  'shauna',
  'shauna-masters',
  'shauntal',
  'shauntal-masters',
  'shelly',
  'shelly-gen3',
  'shielbert',
  'sidney',
  'sidney-gen3',
  'siebold',
  'siebold-masters',
  'sierra',
  'sightseer',
  'sightseerf',
  'silver',
  'silver-gen2',
  'silver-gen2kanto',
  'silver-masters',
  'sina',
  'sina-gen6',
  'sisandbro',
  'sisandbro-gen3',
  'sisandbro-gen3rs',
  'skier',
  'skierf',
  'skierf-gen4dp',
  'skier-gen2',
  'skullgrunt',
  'skullgruntf',
  'skyla',
  'skyla-masters',
  'skyla-masters2',
  'skytrainer',
  'skytrainerf',
  'smasher',
  'soliera',
  'sonia',
  'sonia-masters',
  'sonia-professor',
  'sophocles',
  'sordward',
  'sordward-shielbert',
  'spark',
  'spark-casual',
  'spenser',
  'spenser-gen3',
  'srandjr-gen3',
  'stargruntf-s',
  'stargruntf-v',
  'stargrunt-s',
  'stargrunt-v',
  'steven',
  'steven-gen3',
  'steven-gen6',
  'steven-masters',
  'steven-masters2',
  'steven-masters3',
  'steven-masters4',
  'streetthug',
  'streetthug-masters',
  'striker',
  'supernerd',
  'supernerd-gen1',
  'supernerd-gen1rb',
  'supernerd-gen2',
  'supernerd-gen3',
  'surfer',
  'swimmer',
  'swimmerf',
  'swimmerf2-gen6',
  'swimmerf2-gen7',
  'swimmerf-gen2',
  'swimmerf-gen3',
  'swimmerf-gen3rs',
  'swimmerf-gen4',
  'swimmerf-gen4dp',
  'swimmerf-gen6',
  'swimmerf-gen7',
  'swimmerf-gen8',
  'swimmerfjp-gen2',
  'swimmer-gen1',
  'swimmer-gen1rb',
  'swimmer-gen4',
  'swimmer-gen4dp',
  'swimmer-gen4jp',
  'swimmer-gen6',
  'swimmer-gen7',
  'swimmer-gen8',
  'swimmer-masters',
  'swimmerm-gen2',
  'swimmerm-gen3',
  'swimmerm-gen3rs',
  'sycamore',
  'sycamore-masters',
  'tabitha',
  'tabitha-gen3',
  'tamer-gen1',
  'tamer-gen1rb',
  'tamer-gen3',
  'taohua',
  'tarragon',
  'tate',
  'tateandliza-gen3',
  'tateandliza-gen6',
  'tate-gen6',
  'tate-masters',
  'taunie',
  'teacher',
  'teacher-gen2',
  'teacher-gen7',
  'teamaquabeta-gen3',
  'teamaquagruntf-gen3',
  'teamaquagruntm-gen3',
  'teammagmagruntf-gen3',
  'teammagmagruntm-gen3',
  'teammates',
  'teamrocket',
  'teamrocketgruntf-gen3',
  'teamrocketgruntm-gen3',
  'theroyal',
  'thorton',
  'tierno',
  'tina-masters',
  'toddsnap',
  'toddsnap2',
  'tourist',
  'touristf',
  'touristf2',
  'trace',
  'trevor',
  'trialguide',
  'trialguidef',
  'triathletebikerf-gen3',
  'triathletebiker-gen6',
  'triathletebikerm-gen3',
  'triathleterunnerf-gen3',
  'triathleterunner-gen6',
  'triathleterunnerm-gen3',
  'triathleteswimmerf-gen3',
  'triathleteswimmer-gen6',
  'triathleteswimmerm-gen3',
  'tuber',
  'tuberf',
  'tuberf-gen3',
  'tuberf-gen3rs',
  'tuberf-gen6',
  'tuber-gen3',
  'tuber-gen6',
  'tucker',
  'tucker-gen3',
  'tuli',
  'tulip',
  'turo',
  'turo-ai',
  'twins',
  'twins-gen2',
  'twins-gen3',
  'twins-gen3rs',
  'twins-gen4',
  'twins-gen4dp',
  'twins-gen6',
  'tyme',
  'ultraforestkartenvoy',
  'unknown',
  'unknownf',
  'urbain',
  'valerie',
  'vessa',
  'veteran',
  'veteranf',
  'veteranf-gen6',
  'veteranf-gen7',
  'veteran-gen4',
  'veteran-gen6',
  'veteran-gen7',
  'victor',
  'victor-dojo',
  'victor-league',
  'victor-masters',
  'victor-tundra',
  'vince',
  'viola',
  'viola-masters',
  'volkner',
  'volkner-masters',
  'volo',
  'volo-ginkgo',
  'waiter',
  'waiter-gen4',
  'waiter-gen4dp',
  'waiter-gen9',
  'waitress',
  'waitress-gen4',
  'waitress-gen6',
  'waitress-gen9',
  'wallace',
  'wallace-gen3',
  'wallace-gen3rs',
  'wallace-gen6',
  'wallace-masters',
  'wally',
  'wally-gen3',
  'wally-masters',
  'wally-rse',
  'wattson',
  'wattson-gen3',
  'whitney',
  'whitney-gen2',
  'whitney-masters',
  'wicke',
  'wikstrom',
  'will',
  'will-gen2',
  'willow',
  'willow-casual',
  'winona',
  'winona-gen3',
  'winona-gen6',
  'worker',
  'worker2-gen6',
  'workerf-gen8',
  'worker-gen4',
  'worker-gen6',
  'worker-gen7',
  'worker-gen8',
  'worker-gen9',
  'workerice',
  'worker-lgpe',
  'wulfric',
  'xerosic',
  'yancy',
  'yellgrunt',
  'yellgruntf',
  'yellow',
  'youngathlete',
  'youngathletef',
  'youngcouple',
  'youngcouple-gen3',
  'youngcouple-gen3rs',
  'youngcouple-gen4dp',
  'youngcouple-gen6',
  'youngn',
  'youngster',
  'youngster-gen1',
  'youngster-gen1rb',
  'youngster-gen2',
  'youngster-gen3',
  'youngster-gen3rs',
  'youngster-gen4',
  'youngster-gen4dp',
  'youngster-gen6',
  'youngster-gen6xy',
  'youngster-gen7',
  'youngster-gen8',
  'youngster-gen9',
  'youngster-masters',
  'yukito-hideko',
  'zinnia',
  'zinnia-masters',
  'zinzolin',
  'zirco-unite',
  'zisu',
  'zossie',
];
