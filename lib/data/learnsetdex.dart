...
/// Showdown pokemon IDs that can learn it. Loaded as a pre-built JSON
/// asset (generated by `tools/build_inverse_learnsets.py`) so we avoid
/// re-walking the ~75K (pokemon, move) pairs on every cold launch.
Future<Map<String, List<String>>> loadInverseLearnsets() async {
  if (_inverseLearnsetCache != null) return _inverseLearnsetCache!;
  final raw = await rootBundle.loadString('assets/inverse_learnsets.json');
  final parsed = json.decode(raw) as Map<String, dynamic>;
  _inverseLearnsetCache = parsed.map(
    (k, v) => MapEntry(k, (v as List).cast<String>()),
  );
  return _inverseLearnsetCache!;
}
